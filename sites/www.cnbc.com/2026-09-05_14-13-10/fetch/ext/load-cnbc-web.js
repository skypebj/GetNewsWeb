...
mpscall["cag[geoid]"] = mpscall["cag[geoid]"] || mps._ck.r('geoEdition');
// ZONEID passed as LOCALE see cag envelope keys
var __usNetwork = '2620,5727661';
var __intlNetwork = '5727661';

/* MPS GEO Target - User Geo Data from OneTrust (loader js) Updated 10-16-2025 */
...
mps.geo._get=function(){
  mps._d('[mps:geo] Using OneTrust Geo Data');
  
  // Check if OneTrust is available
  if(typeof(Optanon) !== 'undefined' && typeof(Optanon.getGeolocationData) === 'function') {
    try {
      var oneTrustGeoData = Optanon.getGeolocationData();
      if(oneTrustGeoData && oneTrustGeoData.country) {
        // Map OneTrust format to existing mps.geo.data format
        mps.geo.data = {
          c: oneTrustGeoData.country.toLowerCase(),
          s: oneTrustGeoData.state || '',
          n: 'onetrust', // indicate source
          d: 0 // default value, not provided by OneTrust
        };
        
        mps._d('[mps:geo] Loaded from OneTrust: '+JSON.stringify(mps.geo.data));
        mps.geo._got = 1;
        mps._queue.render('geo',mps.geo.data);
        return mps.geo.data;
      }
    } catch(e) {
      mps._d('[mps:geo] OneTrust Error: ' + e.message);
    }
  }
  
  // Fallback: Set default US data if OneTrust is not available
  mps._d('[mps:geo] OneTrust not available, using default US data');
  mps.geo.data = {
    c: 'us',
    s: '',
    n: 'default',
...
// ZONEID determination - now uses geo data for network selection
var __rawZone = mpscall["cag[zoneid]"];
var __zoneid = '';
if (typeof __rawZone === 'string') {
  __zoneid = __rawZone.trim().toLowerCase();
} else if (__rawZone != null) {
  __zoneid = String(__rawZone).trim().toLowerCase();
}

// Use geo data to determine zone if available and user is not in US
if (mps.geo.data && mps.geo.data.c && mps.geo.data.c !== 'us') {
  __zoneid = 'intl';
  mps._debug('[MPS:LOADER] cag[zoneid] set to "intl" based on geo country: ' + mps.geo.data.c);
} else if (!__zoneid) {
  __zoneid = 'us';
  mps._debug('[MPS:LOADER] cag[zoneid] not set; defaulting to "us"');
} else {
  __zoneid = __zoneid === 'us' ? 'us' : 'intl';
  if (__zoneid !== __rawZone) {
    mps._debug('[MPS:LOADER] normalized cag[zoneid] from "'+ __rawZone +'" to "'+ __zoneid +'"');
  }
}

mps.eol = mps.eol || {};
mps.eol.zone = __zoneid;
mpscall["cag[zoneid]"] = __zoneid;
mps._debug('[MPS:LOADER] cag[zoneid] set to: ' + mpscall["cag[zoneid]"]);
// END ZONEID

// BEGIN NETWORKID - uses zoneid determined above
var __assignedNetwork = (__zoneid === 'us') ? __usNetwork : __intlNetwork;
mpscall["cag[dfpnetwork]"] = __assignedNetwork;
mps._debug('[MPS:LOADER] cag[dfpnetwork] set to: ' + __assignedNetwork + ' based on cag[zoneid] "' + __zoneid + '"');
// END NETWORKID
...
/*=======HB VENDORS=======*/
mps.hb.a9pubid = '3219';
mps.hb.mgid = (mpscall['cag[zoneid]'] === 'us') ? '24610' : '27850';
