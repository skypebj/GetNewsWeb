 ...
 */
mps.inViewport = function(s, map, threshold) {
  ...
  if(!mps._bool(mpsopts.activeload) || (mps._bool(mpsopts.activeload) === true && mps._activerefresh._focus === true)) {
    var win = window, winDocElem = win.document.documentElement, docHeight = mps._getDocHeight(), vp = mps._vp(),
        ...
        elH = s.getBoundingClientRect().height || 0,
        elHOffset = elH * (threshold || 0),
        ...
        vpMinY = (elY + elHOffset) >= ((scY - vpPad) < 0 ? -20 : (scY - vpPad));  //bottom - viewability threshold
        vpMaxY = (elY + elHOffset) <= (scY + vpH + vpPad);  // right - viewability threshold
    if(map) {
...
//--[Console Styling Helpers] 3-pill badge system
// label(color) - Prebid-shaped CSS string for a badge pill
mps.__consoleLabel = function(color) {
  return 'display:inline-block;color:#fff;background:' + color + ';padding:1px 4px;border-radius:3px';
};

// getBrandPillColor() - environment-aware brand pill color (resolved at emit time)
mps.__consoleBrandColor = function() {
  if (typeof mps !== 'undefined' && mps._ext && mps._ext.IRSOURCE) { return '#990099'; }
  var env = (typeof mps_env === 'string') ? mps_env : 'prod';
  var MAP = {
    'prod': '#7598aa',
    'local': '#92400e', 'dev': '#92400e', 'devint': '#92400e',
    'stage': '#990099', 'qa': '#990099',
    'uat': '#be123c',
    'brand': '#c2410c', 'load': '#c2410c',
    'decoupled': '#15803d'
  };
  return MAP[env] || '#64748b';
};

// nsColor(token) - namespace color lookup (2nd pill)
mps.__consoleNsColor = (function() {
  var MAP = {
    'hb': '#0f766e', 'preLoader': '#065f46', 'Rails': '#4338ca',
    'GPT': '#1d4ed8', 'Refresh': '#c2410c', 'timedRefresh': '#ea580c',
    'JS': '#047857', 'Responsive': '#6d28d9', 'Detect': '#7c3aed',
    'SmartRefresh': '#d97706', 'LAV': '#0e7490', 'DestroyAds': '#2563eb',
    'QUEUE': '#475569', 'postLoader': '#059669', 'Loader': '#0d9488',
    'AB': '#64748b'
  };
  return function(token) { return MAP[token] || '#64748b'; };
})();

// subNsColor(token) - sub-namespace color lookup (3rd pill)
mps.__consoleSubNsColor = (function() {
  var MAP = {
    'Core': '#3730a3', 'mg': '#be123c', 'a9': '#b45309',
    'cmp': '#0e7490', 'dv': '#4d7c0f', 'ae': '#a21caf'
  };
  return function(token) { return MAP[token] || '#64748b'; };
})();

// getLogContext(msg) - parse leading [mps:ns:sub] prefix from message body
// Returns {ns: string|null, sub: string|null, body: string}
mps.__consoleParseContext = function(msg) {
  var result = { ns: null, sub: null, body: msg };
  var match = msg.match(/^\[([^\]]+)\]\s*/);
  if (!match) { return result; }
  var segments = match[1].split(':');
  var i = 0;
  // Drop leading 'mps' segment if present
  if (segments[0] === 'mps') { i = 1; }
  // Sanitize: alphanumeric, underscore, hyphen; max 16 chars; skip lone severity chars
  var SEVERITY_CHARS = { '!': 1, '^': 1, '#': 1, '~': 1 };
  var clean = function(s) {
    s = s.replace(/^\s+|\s+$/g, '').replace(/[^A-Za-z0-9_-]/g, '').substring(0, 16);
    return (s.length === 0 || SEVERITY_CHARS[s]) ? null : s;
  };
  if (i < segments.length) { result.ns = clean(segments[i]); i++; }
  if (i < segments.length) { result.sub = clean(segments[i]); }
  result.body = msg.substring(match[0].length);
  return result;
};

// decorateLog(plainBody, severity) - build styled console args
// severity: 'log'|'debug'|'warn'|'error'
// Returns {method: string, args: array, plainBody: string, levelWord: string}
mps.__consoleDecorate = function(plainBody, severity) {
  var ctx = mps.__consoleParseContext(plainBody);
  var LEVEL_WORDS = { 'log': 'MESSAGE:', 'debug': 'DEBUG:', 'warn': 'WARNING:', 'error': 'ERROR:' };
  var levelWord = LEVEL_WORDS[severity] || 'MESSAGE:';
  var brandColor = mps.__consoleBrandColor();
  var label = mps.__consoleLabel;

  // Build format string and CSS args
  var fmt = '%cmps:';
  var styles = [label(brandColor)];

  if (ctx.ns) {
    fmt += '%c' + ctx.ns;
    styles.push(label(mps.__consoleNsColor(ctx.ns)));
  }
  if (ctx.sub) {
    fmt += '%c' + ctx.sub;
    styles.push(label(mps.__consoleSubNsColor(ctx.sub)));
  }

  // Severity stripe: debug gets faint violet bg; others empty (browser-native for warn/error)
  var stripe = (severity === 'debug') ? 'background:rgba(109,40,217,0.08);padding:2px 4px;border-radius:2px' : '';
  fmt += '%c ' + levelWord + ' ' + ctx.body;
  styles.push(stripe);

  return { method: severity === 'debug' ? 'debug' : severity, args: [fmt].concat(styles), plainBody: ctx.body, levelWord: levelWord };
};
...
mps.__console = {
  log: function() {
    ...
    while (args[i_arg]){
      ...
      if(window.console && console.log && console.warn && console.debug && console.error) {
        ...
        var f = m.charAt(0);
        var severity, plainMsg;
        if(f=='~') {
          severity = 'log'; plainMsg = m.substring(1);
          mps.__console._last = false;
        } else if(f=='!') {
          severity = 'error'; plainMsg = m.substring(1);
          mps.__console._last = 'error';
        } else if(f=='^') {
          severity = 'warn'; plainMsg = m.substring(1);
          mps.__console._last = 'warn';
        } else if(f=='#') {
          severity = 'debug'; plainMsg = m.substring(1);
          mps.__console._last = 'debug';
        } else {
          mps.__console.auto(m);
          continue;
        }
        var decorated = mps.__consoleDecorate(plainMsg, severity);
        console[decorated.method].apply(console, decorated.args);
        mps.__console.overlay(plainMsg, severity);
  ...
  debug: function() {
    if(!mps._isdebug() || mps.__nolog) {
      return false;
    }
    // Emit directly — do not delegate to log to prevent double-emit.
    // Uses console.log (not console.debug) so messages appear at "Default levels"
    // in Chrome DevTools. The _isdebug() gate above already controls visibility;
    // the DEBUG: level word + violet stripe provide visual distinction.
    var args = Array.prototype.slice.call(arguments);
    var i_arg = 0;
    while (args[i_arg]) {
      var m = args[i_arg];
      ...
      if(window.console && console.log) {
        ...
        // Respect severity markers; default to 'debug' when no marker present
        var f = m.charAt(0);
        var MARKER_MAP = { '!': 'error', '^': 'warn', '#': 'debug', '~': 'log' };
        var severity = MARKER_MAP[f] || 'debug';
        var plainMsg = MARKER_MAP[f] ? m.substring(1) : m;
        var decorated = mps.__consoleDecorate(plainMsg, severity);
        // Use console.log as floor — never console.debug (hidden at "Default levels")
        var method = (decorated.method === 'debug') ? 'log' : decorated.method;
        console[method].apply(console, decorated.args);
        mps.__console.overlay(plainMsg, severity);
      }
    }
    return true;
...
mps._gptloadCallback = function(){
  ...
  // Flush any mpsready items that were queued after the initial mpsCallback flush.
  // On deferred-execution environments (e.g. Next.js/Bento, nowrite>=2), the initial
  // mpsready flush in mpsCallback runs before all queue items are pushed, leaving
  // late items (including HB initialization) stranded. Re-flushing here — after GPT
  // slot definitions are complete — catches them. On standard sites the queue is
...
mps._sanitizeQS = function(str) {
  try {
    str = String(str || '');
    // Replace + with space then decode
    str = str.replace(/\+/g, '%20');
    try { str = decodeURIComponent(str); } catch (e) {}
    // Allow only RFC3986 unreserved and a small safe subset
    return str.replace(/[^A-Za-z0-9._~!$&'()*+,;=:@\/-]/g, '');
  } catch (e) { return ''; }
};

mps._sanitizePathSeg = function(seg) {
  try {
    seg = String(seg || '');
    // Strict: keep alphanum, dash, underscore, dot, tilde
    seg = seg.replace(/[^A-Za-z0-9._~-]/g, '');
    // Prevent reserved dot names
    if (seg === '.' || seg === '..') return '';
    return seg;
  } catch (e) { return ''; }
};
...
mps._debug('[mps:preLoader] mpsRequestParams()');
  ...
  if(sitepath!='' && sitepath!='/' && sitepath.indexOf('/') > -1) {
    ...
    for (var i=0; i<sitepatharr.length; i++) {
      var _seg = mps._sanitizePathSeg(sitepatharr[i]);
      mps._ext._pathsegs[i+1] = _seg;
      if(i < mpsopts.maxpathsegs) {
        cleanpatharr[i+1]= _seg;
      } else {
        cutpatharr[i+1] = _seg;
      }
    }
  ...
  for(var i=0; i<qs.length; i++){
    qsv = qs[i].split('=');
    if(typeof(qsv[0])!='undefined' && qsv[0] !== '') {
      var _k = mps._sanitizeQS(qsv[0]);
      var _v = (typeof(qsv[1])!='undefined') ? mps._sanitizeQS(qsv[1]) : '';
      if(_k) mps._ext._qsparams[_k] = _v;
    }
  }
  return mpscall;
};
  ...
  if(mps._ext.nowrite > 0 && !mps.__intcode && !mpsopts.skipautorequest) {
    ...
    if(mpsopts.skipheader != 1) {
      if((!mps._select('body') && !mps.__intcode) || mps._ext.mpsready === true || typeof mps._postscribe !== 'function') {
        ...
        // Avoid document.write sink; append when body is available (raw insert)
        mps._append(mps._select('body'), mps.response.pagevars.insert_bodyheader);
      }
      mps._ext.loadheader = 1;
    }
...
mps.writeFooter = function() {
  ...
  if(mps._ext.nowrite !== 1 && _loadfooter === false) {
    ...
    mps._debug('[mps:preLoader] mps.writeFooter: DOCUMENT.WRITE');
      // Avoid document.write sink; append safely to body
...
// set isapp
mps.app = mps.app || { isApp: false, name: null, os: null };
try {
  mps.app.isApp = mps._bool(mps._get('chromeless'));
  mpscall.isapp = mps.app.isApp;
  mpscall['cag[isapp]'] = mps.app.isApp ? 'true' : 'false';
  if (mps.app.isApp) {
    var adunit = mpscall.adunit || '';
    var ua = navigator.userAgent || '';
    if (/\/today(?:\/|$)/i.test(adunit)) mps.app.name = 'today';
    else if (/\/nbcnews(?:\/|$)/i.test(adunit)) mps.app.name = 'nbcnews';
    if (/android/i.test(ua)) mps.app.os = 'android';
    else if (/iphone|ipad|ipod/i.test(ua)) mps.app.os = 'ios';
  }
} catch (e) {}
...
mps.hb = mps.hb || {}; 
mps.hb=mps.hb||{};
// [2022-12-15] fix endless refreshes on responsiveapply
mpsopts=mpsopts||{};
mpsopts.updatecorrelator=1;
...
mps.hb = mps.hb || {};
mps.hb.executed = 0;
mps.hb._version = 10.6;
mps.hb._core = true;
/** Category exclusion value used to opt a slot out of header bidding */
mps.hb.CAT_EXCLUSION_HB = 'headerbid';
...
mps.hb.cmptimeout = mps.hb.cmptimeout || 3000;
mps.hb.kvinsertcheckinterval = mps.hb.kvinsertcheckinterval || 250;
mps.hb.initialslotretryinterval = mps.hb.initialslotretryinterval || 125;
mps.hb.initialslotretrylimit = mps.hb.initialslotretrylimit || 8;
mps.hb.a9sisectoverride = mps.hb.a9sisectoverride || '';
mps.hb.loadedads = mps.hb.loadedads || 0;
mps.hb.cmpenabled = mps.hb.cmpenabled !== undefined ? mps.hb.cmpenabled : false;
mps.hb.cmpprivacyregionsonly = mps.hb.cmpprivacyregionsonly !== undefined ? mps.hb.cmpprivacyregionsonly : false;
mps.hb.cmpregion = typeof mps.hb.cmpregion === 'string' ? mps.hb.cmpregion : '';
mps.hb.gdpr_countries = Array.isArray(mps.hb.gdpr_countries) ? mps.hb.gdpr_countries : [
  'AT', 'BE', 'BG', 'HR', 'CY', 'CZ', 'DK', 'EE', 'FI', 'FR', 'DE', 'GR', 'HU', 'IS', 'IE', 'IT',
  'LV', 'LI', 'LT', 'LU', 'MT', 'NL', 'NO', 'PL', 'PT', 'RO', 'SK', 'SI', 'ES', 'SE', 'GB', 'CH'
];
mps.hb._cmpready = mps.hb._cmpready || false;
mps.hb._cmpreadyresolved = mps.hb._cmpreadyresolved || false;
mps.hb._cmpwaitstarted = mps.hb._cmpwaitstarted || false;
mps.hb._cmpflowstarted = mps.hb._cmpflowstarted || false;
mps.hb._cmpreadyqueue = mps.hb._cmpreadyqueue || [];
...
mps.hb.mgresetonwin = mps.hb.mgresetonwin !== undefined ? mps.hb.mgresetonwin : true;
mps.hb.useBidCache = mps.hb.useBidCache !== undefined ? mps.hb.useBidCache : true;
mps.hb.refreshbids = mps.hb.refreshbids !== undefined ? mps.hb.refreshbids : true;
mps.hb.nohb.refresh = mps.hb.nohb.refresh || [];

mps.hb.markTiming = function(name, value) {
  mps._reqs.hb = mps._reqs.hb || {};
  mps._reqs.hb[name] = value;
  return value;
};

mps.hb.isCmpReady = function() {
  if (mps.hb._cmpreadyresolved || mps.hb._cmpready) {
    return true;
  }
  const hasOptanonGroups = globalThis.OptanonActiveGroups !== undefined;
  const hasOptanonCookie = !!mps._ck?.r('OptanonConsent');
  return hasOptanonGroups || hasOptanonCookie;
};

mps.hb.getPrivacyRegionInfo = function() {
  const regionFields = [
    { source: 'mps.response.dart.params.country', value: mps.response?.dart?.params?.country },
    { source: 'mps.response.pagevars.country', value: mps.response?.pagevars?.country },
    { source: 'mps.pagevars.country', value: mps.pagevars?.country },
    { source: 'mpscall.country', value: mpscall?.country },
    { source: 'mps.response.dart.params.gdpr_region', value: mps.response?.dart?.params?.gdpr_region },
    { source: 'mps.response.dart.params.privacy_region', value: mps.response?.dart?.params?.privacy_region },
    { source: 'mps.response.dart.params.region', value: mps.response?.dart?.params?.region },
    { source: 'mps.response.pagevars.gdpr_region', value: mps.response?.pagevars?.gdpr_region },
    { source: 'mps.response.pagevars.privacy_region', value: mps.response?.pagevars?.privacy_region },
    { source: 'mps.response.pagevars.region', value: mps.response?.pagevars?.region },
    { source: 'mps.pagevars.gdpr_region', value: mps.pagevars?.gdpr_region },
    { source: 'mps.pagevars.privacy_region', value: mps.pagevars?.privacy_region },
    { source: 'mps.pagevars.region', value: mps.pagevars?.region },
    { source: 'mpscall.gdpr_region', value: mpscall?.gdpr_region },
    { source: 'mpscall.privacy_region', value: mpscall?.privacy_region },
    { source: 'mpscall.region', value: mpscall?.region }
  ];
  const gdprCountries = mps.hb.gdpr_countries.map(function(code) {
    return typeof code === 'string' ? code.trim().toUpperCase() : '';
  }).filter(function(code) {
    return code.length === 2;
  });
  const privacyRegionInfo = {
    isPrivacyRegion: false,
    source: '',
    rawValue: '',
    normalizedValue: '',
    resolvedCountryCode: '',
    gdprMatchedCountryCode: ''
  };
  mps.hb.cmpregion = '';
  regionFields.some(function(field) {
    if (typeof field.value !== 'string' || field.value.length === 0) {
      return false;
    }
    const normalizedValue = field.value.trim().toUpperCase();
    const countryMatches = normalizedValue.match(/\b[A-Z]{2}\b/g) || [];
    const resolvedCountryCode = /^[A-Z]{2}$/.test(normalizedValue) ? normalizedValue : (countryMatches[0] || '');
    if (!privacyRegionInfo.source) {
      privacyRegionInfo.source = field.source;
      privacyRegionInfo.rawValue = field.value;
      privacyRegionInfo.normalizedValue = normalizedValue;
    }
    ...
    privacyRegionInfo.rawValue = field.value;
    privacyRegionInfo.normalizedValue = normalizedValue;
    privacyRegionInfo.resolvedCountryCode = resolvedCountryCode;
    privacyRegionInfo.isPrivacyRegion = gdprCountries.includes(resolvedCountryCode);
    privacyRegionInfo.gdprMatchedCountryCode = privacyRegionInfo.isPrivacyRegion ? resolvedCountryCode : '';
    mps.hb.cmpregion = resolvedCountryCode;
    if (privacyRegionInfo.isPrivacyRegion && privacyRegionInfo.gdprMatchedCountryCode) {
      mps.pagevars = mps.pagevars || {};
      mps.pagevars.region = privacyRegionInfo.gdprMatchedCountryCode;
    }
    return true;
  });
  return privacyRegionInfo;
};

mps.hb.isPrivacyRegion = function() {
  return mps.hb.getPrivacyRegionInfo().isPrivacyRegion;
};

mps.hb.getCmpGateDecision = function() {
  const cmpEnabled = !!mps.hb.cmpenabled;
  const cmpPrivacyRegionsOnly = !!mps.hb.cmpprivacyregionsonly;
  if (!cmpEnabled) {
    return {
      enabled: false,
      reason: 'cmpenabled=false',
      privacyRegionRan: false,
      privacyRegionInfo: null
    };
  }
  if (!cmpPrivacyRegionsOnly) {
    return {
      enabled: true,
      reason: 'cmpenabled=true and cmpprivacyregionsonly=false',
      privacyRegionRan: false,
      privacyRegionInfo: null
    };
  }
  const privacyRegionInfo = mps.hb.getPrivacyRegionInfo();
  return {
    enabled: privacyRegionInfo.isPrivacyRegion,
    reason: privacyRegionInfo.isPrivacyRegion ? 'cmpenabled=true and cmpprivacyregionsonly=true with privacy-region match' : 'cmpenabled=true and cmpprivacyregionsonly=true without privacy-region match',
    privacyRegionRan: true,
    privacyRegionInfo: privacyRegionInfo
  };
};

mps.hb.isCmpEnabled = function() {
  return mps.hb.getCmpGateDecision().enabled;
};

mps.hb.waitForCmpReady = function(callback) {
  const onReady = typeof callback === 'function' ? callback : function() {};
  if (mps.hb.isCmpReady()) {
    mps.hb._cmpready = true;
    mps.hb.markTiming('cmp_wait_ms', 0);
    onReady();
    return true;
  }
  mps.hb._cmpreadyqueue = mps.hb._cmpreadyqueue || [];
  mps.hb._cmpreadyqueue.push(onReady);
  if (mps.hb._cmpwaitstarted) {
    return true;
  }
  mps.hb._cmpwaitstarted = true;
  const cmpWaitStartTs = Date.now();
  let cmpReadyInterval = null;
  let cmpReadyTimeout = null;
  let tcfListenerId = null;
  const finishCmpWait = function(source) {
    if (mps.hb._cmpreadyresolved) {
      return;
    }
    mps.hb._cmpreadyresolved = true;
    mps.hb._cmpready = true;
    mps.hb._cmpwaitstarted = false;
    if (cmpReadyInterval) {
      clearInterval(cmpReadyInterval);
    }
    if (cmpReadyTimeout) {
      clearTimeout(cmpReadyTimeout);
    }
    globalThis.removeEventListener('OneTrustGroupsUpdated', onOneTrustGroupsUpdated);
    if (tcfListenerId && typeof globalThis.__tcfapi === 'function') {
      try {
        globalThis.__tcfapi('removeEventListener', 2, function() {}, tcfListenerId);
      } catch (tcfRemoveErr) {
        mps._d('[mps:hb:cmp] removeEventListener error: ' + tcfRemoveErr.message);
      }
    }
    const cmpWaitMs = Date.now() - cmpWaitStartTs;
    mps.hb.markTiming('cmp_wait_ms', cmpWaitMs);
    mps._d('[mps:hb:cmp] CMP gate released via ' + source + ' after ' + cmpWaitMs + 'ms');
    const readyQueue = mps.hb._cmpreadyqueue || [];
    mps.hb._cmpreadyqueue = [];
    while (readyQueue.length) {
      try {
        readyQueue.shift().call();
      } catch (cmpReadyErr) {
        mps._errorCallback(cmpReadyErr, 'mps.hb.waitForCmpReady');
      }
    }
  };
  const onOneTrustGroupsUpdated = function() {
    if (mps.hb.isCmpReady()) {
      finishCmpWait('OneTrustGroupsUpdated');
    }
  };
  globalThis.addEventListener('OneTrustGroupsUpdated', onOneTrustGroupsUpdated);
  if (typeof globalThis.__tcfapi === 'function') {
    try {
      globalThis.__tcfapi('addEventListener', 2, function(tcData, success) {
        if (tcData?.listenerId) {
          tcfListenerId = tcData.listenerId;
        }
        if (success && (tcData?.eventStatus || tcData?.tcString || mps.hb.isCmpReady())) {
          finishCmpWait('tcfapi');
        }
      });
    } catch (tcfErr) {
      mps._d('[mps:hb:cmp] __tcfapi addEventListener error: ' + tcfErr.message);
    }
  }
  cmpReadyInterval = setInterval(function() {
    if (mps.hb.isCmpReady()) {
      finishCmpWait('fallback-poll');
    }
  }, 100);
  cmpReadyTimeout = setTimeout(function() {
    mps._d('[mps:hb:cmp] CMP wait timed out after ' + mps.hb.cmptimeout + 'ms; continuing');
    finishCmpWait('timeout');
  }, mps.hb.cmptimeout);
  return true;
};

/**
 * Slot-to-bid snapshot data structures.
 * Written by _snapshotSlotBid at each GPT refresh point.
 * Read by the prebid-debugger bookmarklet for deterministic slot→bid mapping.
 *
 * Only initialized when debug mode is active (__debugmode__ cookie) to avoid
 * allocating memory for data structures that are only consumed by the debugger.
 *
 * _slotBidMap: { [slotElementId]: latestSnapshot } — current state per slot
 * _slotBidHistory: { [slotElementId]: [snapshot, ...] } — capped timeline per slot
 */
if (mps._isdebug()) {
  mps.hb._slotBidMap = mps.hb._slotBidMap || {};
  mps.hb._slotBidHistory = mps.hb._slotBidHistory || {};

  /** Max history entries per slot (prevents unbounded memory growth) */
  mps.hb._slotBidHistoryMax = 50;

  /**
   * _snapshotSlotBid
   * Captures the current Prebid targeting from a GPT slot object and writes
   * it to _slotBidMap (latest) and _slotBidHistory (timeline).
   *
   * Must be called AFTER Prebid sets targeting but BEFORE googletag.pubads().refresh()
   * consumes it — i.e. at each of the 4 refresh points in the HB code.
   *
   * @param {object} slotObj - GPT slot object (has getSlotElementId, getAdUnitPath, getTargetingMap)
   * @param {string} source - one of 'initial', 'adexecute', 'refresh', 'refreshOverride'
   */
  mps.hb._snapshotSlotBid = function(slotObj, source) {
    try {
      if (!slotObj || typeof slotObj.getSlotElementId !== 'function') {
        return;
      }
      const id = slotObj.getSlotElementId();
      const targeting = typeof slotObj.getTargetingMap === 'function' ? slotObj.getTargetingMap() : {};
      const cmpGateDecision = typeof mps.hb.getCmpGateDecision === 'function' ? mps.hb.getCmpGateDecision() : null;
      const privacyRegionInfo = cmpGateDecision && cmpGateDecision.privacyRegionInfo
        ? cmpGateDecision.privacyRegionInfo
        : (typeof mps.hb.getPrivacyRegionInfo === 'function' ? mps.hb.getPrivacyRegionInfo() : null);
      mps.hb['_lastCmpGateDecision'] = cmpGateDecision || mps.hb['_lastCmpGateDecision'] || null;
      if (privacyRegionInfo) {
        mps.hb['_lastPrivacyRegionInfo'] = privacyRegionInfo;
      }
      const snapshot = {
        slotElementId: id,
        adUnitPath: typeof slotObj.getAdUnitPath === 'function' ? slotObj.getAdUnitPath() : '',
        source: source,
        timestamp: Date.now(),
        loadset: mps._loadset || 0,
        hb_adid: (targeting.hb_adid || [''])[0],
        hb_bidder: (targeting.hb_bidder || [''])[0],
        hb_pb: (targeting.hb_pb || [''])[0],
        hb_auction: (targeting.hb_auction || [''])[0],
        cmpGateEnabled: !!(cmpGateDecision && cmpGateDecision.enabled),
        cmpGateReason: cmpGateDecision && cmpGateDecision.reason ? cmpGateDecision.reason : '',
        cmpReady: typeof mps.hb.isCmpReady === 'function' ? !!mps.hb.isCmpReady() : false,
        privacyRegionRan: !!(cmpGateDecision && cmpGateDecision.privacyRegionRan),
        isPrivacyRegion: !!(privacyRegionInfo && privacyRegionInfo.isPrivacyRegion),
        privacyRegionSource: privacyRegionInfo && privacyRegionInfo.source ? privacyRegionInfo.source : '',
        privacyRegionRawValue: privacyRegionInfo && privacyRegionInfo.rawValue ? privacyRegionInfo.rawValue : '',
        gdprCountryCode: privacyRegionInfo && privacyRegionInfo.gdprMatchedCountryCode ? privacyRegionInfo.gdprMatchedCountryCode : ''
      };

      // Capture all bid responses for this ad unit at snapshot time.
      // This preserves bid data that Prebid may later evict from its pool,
      // giving the debugger a timing-proof record of every bid.
      try {
        if (typeof pbjs !== 'undefined' && typeof pbjs.getBidResponsesForAdUnitCode === 'function') {
          const adUnitCode = snapshot.adUnitPath;
          const bidBlock = pbjs.getBidResponsesForAdUnitCode(adUnitCode);
          const bids = bidBlock && Array.isArray(bidBlock.bids) ? bidBlock.bids : [];
          snapshot.allBids = bids.map(function(b) {
            return {
              ...
              size: (b.width && b.height) ? b.width + 'x' + b.height : b.size || '',
              timeToRespond: b.timeToRespond || 0,
              mediaType: b.mediaType || 'banner',
              dealId: b.dealId || '',
              source: b.source || 'client'
            };
          });
        }
      } catch (bidErr) {
        mps._d('[mps:hb] _snapshotSlotBid allBids capture error: ' + bidErr.message);
      }

      // Write latest
      mps.hb._slotBidMap[id] = snapshot;

      // Append to history (capped)
      mps.hb._slotBidHistory[id] = mps.hb._slotBidHistory[id] || [];
      mps.hb._slotBidHistory[id].push(snapshot);
      if (mps.hb._slotBidHistory[id].length > mps.hb._slotBidHistoryMax) {
        mps.hb._slotBidHistory[id] = mps.hb._slotBidHistory[id].slice(-mps.hb._slotBidHistoryMax);
      }

      mps._d('[mps:hb] _snapshotSlotBid: ' + id + ' (' + source + ') hb_bidder=' + snapshot.hb_bidder + ' allBids=' + (snapshot.allBids ? snapshot.allBids.length : 0));
    } catch (err) {
      mps._d('[mps:hb] _snapshotSlotBid error: ' + err.message);
      mps._errorCallback(err, 'mps.hb._snapshotSlotBid');
    }
  };
  mps._d('[mps:hb] Debug mode active — slot bid snapshots enabled');
} else {
  // No-op when not in debug mode — call sites don't need guards
  mps.hb._snapshotSlotBid = function() {};
}
...
mps.hb.executeCallCore = function() {
  mps._d('[mps:hb] Starting Execution v' + mps.hb._version + ' (CORE)');
  // Load DV
  if (mps.hb.dvid) {
    mps.hb.loadDV();
  }
  ...
  // Register bidWon handler and bid cache config once here (prevents duplicate listeners)
  if (mps.hb.mgid) {
    pbjs.que.push(function() {
      if (mps.hb.useBidCache) {
        pbjs.setConfig({
          useBidCache: true,
          bidCacheFilterFunction: function(bid) { return bid.mediaType !== 'video'; },
          auctionOptions: { suppressStaleRender: true }
        });
        mps._d('[mps:hb:mg] Bid cache enabled (video excluded, suppressStaleRender on)');
      }
      pbjs.onEvent('bidWon', mps.hb.bidWonHandler);
    });
  }

  /**
   * Refresh-time bid path #1 of 2: mps.refreshAds pre-hook.
   *
   * Intercepts mps.refreshAds() BEFORE googletag.pubads().refresh() fires.
   * Sends eligible slots to a new Magnite auction; once bids return, it
   * calls googletag.pubads().refresh() itself. This ensures refreshed ads
   * always carry fresh Prebid bids.
   *
   * Contrast with refreshOverrideCallback (path #2) which handles
   * LATE-RENDERED slots that missed the initial page-load auction.
   *
   * Can be bypassed per-call via mps.refreshAds(..., true) (_skiphb flag).
   * Controlled by mps.hb.refreshbids (default: true).
   */
  if (mps.hb.refreshbids && mps.hb.mgid) {
    mps.refreshAdsPreCallback = function(_updatedslotvars, gptopts) {
      if (!pbjs?.libLoaded) {
        mps._d('[mps:hb:mg] Refresh pre-hook: pbjs not loaded, falling through');
        return false;
      }
      if (!_updatedslotvars?.length) {
        return false;
      }
      // Filter out excluded slots
      let slotsForBids = _updatedslotvars.filter(function(slot) {
        let elId = slot.getSlotElementId();
        let name = elId.replace('div-gpt-', '').replace(/-[0-9]+$/, '');
        let catex = slot.getCategoryExclusions ? slot.getCategoryExclusions() : [];
        return !mps.hb.nohb.refresh.includes(name)
            && !catex.includes(mps.hb.CAT_EXCLUSION_HB);
      });
      if (!slotsForBids.length) {
        mps._d('[mps:hb:mg] Refresh pre-hook: all slots excluded, falling through');
        return false;
      }
      mps._d('[mps:hb:mg] Refresh pre-hook: requesting bids for ' + slotsForBids.length + ' slot(s)');
      pbjs.que.push(function() {
        pbjs.rp.requestBids({
          gptSlotObjects: slotsForBids,
          callback: function() {
            try {
              mps._d('[mps:hb:mg] Refresh pre-hook: bids received, refreshing GPT');
              // Clear stale hb_* targeting and apply fresh bids
              pbjs.setTargetingForGPTAsync();
              // Snapshot bid targeting before GPT consumes it
              for (const slot of slotsForBids) {
                mps.hb._snapshotSlotBid(slot, 'refresh');
              }
              googletag.pubads().refresh(slotsForBids, gptopts);
              // Refresh any excluded slots normally (they were filtered out above)
              let excludedSlots = _updatedslotvars.filter(function(s) {
                return !slotsForBids.includes(s);
              });
              if (excludedSlots.length) {
                for (const slot of excludedSlots) {
                  mps.hb._snapshotSlotBid(slot, 'refresh-excluded');
                }
                googletag.pubads().refresh(excludedSlots, gptopts);
              }
            } catch (err) {
              mps._d('^[mps:hb:mg] Refresh pre-hook callback error: ' + err.message);
              mps._errorCallback(err, 'mps.hb.refreshPreHook.callback');
            }
          }
        });
      });
    ...
    mps._d('[mps:hb:mg] Refresh pre-hook registered');
  }
...
mps.hb.executeCall = function() {
  ...
  mps.hb._cmpflowstarted = false;
  mps.hb._executionStartTs = Date.now();
  mps.hb.markTiming('cmp_wait_ms', 0);
  mps.hb.markTiming('mg_load_wait_ms', 0);
  mps.hb.markTiming('auction_start_delay_ms', 0);
  const cmpGateDecision = mps.hb.getCmpGateDecision();
  const privacyRegionInfo = cmpGateDecision.privacyRegionInfo
    ? cmpGateDecision.privacyRegionInfo
    : (typeof mps.hb.getPrivacyRegionInfo === 'function' ? mps.hb.getPrivacyRegionInfo() : null);
  mps.hb['_lastCmpGateDecision'] = cmpGateDecision;
  if (privacyRegionInfo) {
    mps.hb['_lastPrivacyRegionInfo'] = privacyRegionInfo;
    mps.hb.cmpregion = privacyRegionInfo.resolvedCountryCode || '';
  }
  mps._d('[mps:hb:cmp] cmpenabled=' + (!!mps.hb.cmpenabled) + ' cmpprivacyregionsonly=' + (!!mps.hb.cmpprivacyregionsonly));
  if (cmpGateDecision.privacyRegionRan && privacyRegionInfo) {
    mps._d('[mps:hb:cmp] isPrivacyRegion()=' + privacyRegionInfo.isPrivacyRegion + ' source=' + privacyRegionInfo.source + ' raw="' + privacyRegionInfo.rawValue + '" cmpregion=' + (privacyRegionInfo.resolvedCountryCode || 'n/a') + ' gdprCountryCode=' + (privacyRegionInfo.gdprMatchedCountryCode || 'n/a'));
  } else if (privacyRegionInfo && privacyRegionInfo.source) {
    mps._d('[mps:hb:cmp] region lookup source=' + privacyRegionInfo.source + ' raw="' + privacyRegionInfo.rawValue + '" cmpregion=' + (privacyRegionInfo.resolvedCountryCode || 'n/a') + ' gdprCountryCode=' + (privacyRegionInfo.gdprMatchedCountryCode || 'n/a') + ' (' + cmpGateDecision.reason + ')');
  } else {
    mps._d('[mps:hb:cmp] isPrivacyRegion() skipped (' + cmpGateDecision.reason + ')');
  }
  if (!cmpGateDecision.enabled) {
    mps._d('[mps:hb:cmp] Not blocked by CMP (' + cmpGateDecision.reason + ')');
    return mps.hb.executeCallCore();
  }
  mps._d('[mps:hb:cmp] Locked by CMP (' + cmpGateDecision.reason + ')');
  return mps.hb.waitForCmpReady(function() {
    if (!mps.hb._cmpflowstarted) {
      mps.hb._cmpflowstarted = true;
      mps.hb.executeCallCore();
    }
  });
};
/**
...
mps.hb.loadDV = function() {
  try {
    ...
    setTimeout(()=> {
      mps.hb.onDvtagReady(function () { mps._d('[mps:hb:dv] Call onDvtagReady'); });
    },mps.hb.dvtimeout);
    ...
    mps._queue.adexecute.push(function () {
      ...
      const dvValidSlots = mps.hb.getValidSlotObjects(Object.keys(mps._advars[loadset]), loadset);
      for (const { name, adobj } of dvValidSlots) {
        const exclude = adobj?.getCategoryExclusions();
        if (exclude?.includes(mps.hb.CAT_EXCLUSION_HB)) {
          mps._d('[mps:hb:dv] DV: skip cat exclusion of headerbidding ' + adobj.getCategoryExclusions());
          continue;
        }
        if (!adobj?.setTargeting) {
          continue;
        }
        mps._d('[mps:hb:dv] DV: pushing dv to slot ' + adobj.getSlotId());
        mps.hb.dvslots.push(name);
      }

    });
  } catch (err) {
    mps._d('[mps:hb:dv] DV: error - ' + err.message);
...
/**
 * mps.hb.loadMG
 * Loads the Magnite (Prebid) script
 * @return {boolean}
 */
mps.hb.loadMG = function () {
  ...
  try {
    ...
    if (!mps.hb._mgready) {
      const mgLoadStartTs = Date.now();
      let mgLoadHandled = false;
      mps.hb.mgurl = mps.hb.mgurl || '//micro.rubiconproject.com/prebid/dynamic/' + mps.hb.mgid + '.js';
      mps._reqs.hb.mg_begin = mps._elapsed('', true);
      const finalizeMgLoad = function(source) {
        if (mgLoadHandled) {
          return;
        }
        mgLoadHandled = true;
        mps.hb._mgready = 1;
        mps.hb.markTiming('mg_load_wait_ms', Date.now() - mgLoadStartTs);
        if (source === 'timeout') {
      ...
      mps._loadJS(mps.hb.mgurl, function () {
        ...
        finalizeMgLoad('loaded');
      });
      setTimeout(function () {
        if (!mps.hb.adserverCalled && !mps.hb._mgready) {
          finalizeMgLoad('timeout');
        }
...
mps.hb.loadA9 = function() {
  try {
    ...
    let a9sisect = mpscall.cat ? mpscall.cat : 'PAGE';
    ...
    mps._d('[mps:hb:a9] A9 initialized: (' + mps.hb.a9pubid + ') ' + mps._elapsed());
  } catch (err) {
    mps._d('^[mps:hb:a9] A9: error loading - ' + err.message);
    mps._errorCallback(err, 'mps.hb.loadA9');
  }
};
/**
 * mps.hb.getCalledSlotNames
 * Return slot names that have been executed for the current loadset
 * @param {number} loadset
 * @return {Array}
 */
mps.hb.getCalledSlotNames = function(loadset) {
  const calledslots = [];
  if (!mps._slotscalled?.[loadset]) {
    mps._d('[mps:hb] getCalledSlotNames: no slots called for loadset ' + loadset);
    return calledslots;
  }
  for (const slotname of Object.keys(mps._slotscalled[loadset])) {
    calledslots.push(slotname);
  }
  mps._d('[mps:hb] getCalledSlotNames: ' + calledslots.length + ' slot(s) for loadset ' + loadset);
  return calledslots;
};
/**
 * mps.hb.getValidSlotObjects
 * Shared slot resolution — resolves slot names to GPT ad objects,
 * filtering out _oop slots, missing advars, and adobj instances
 * that lack the required GPT slot methods.
 *
 * Used by DV, A9, and startAdExecution to avoid duplicating
 * the same guard logic in each vendor loop.
 *
 * @param {Array<string>} slotNames - slot names to resolve
 * @param {number} loadset - current loadset index
 * @return {Array<{name: string, adobj: object}>} valid slot pairs
 */
mps.hb.getValidSlotObjects = function(slotNames, loadset) {
  const results = [];
  for (const name of slotNames) {
    if (name === '_oop') {
      continue;
    }
    if (!mps._advars[loadset]?.[name]) {
      continue;
    }
    const adobj = gpt[mps._advars[loadset][name]];
    if (!adobj?.getSlotElementId || !adobj?.getAdUnitPath || !adobj?.getSizes || !adobj?.getTargetingMap) {
      continue;
    }
    results.push({ name, adobj });
  }
  return results;
};
/**
 * mps.hb.a9InitBids
 * Wait for executed slots before building A9 requests
 * @param {number} retrycount
 * @return {boolean}
 */
mps.hb.a9InitBids = function(retrycount) {
  const loadset = mps._loadset || 0;
  const calledslots = mps.hb.getCalledSlotNames(loadset);
  retrycount = retrycount || 0;
  mps._d('[mps:hb:a9] A9: Assembling ad slot data');
  if (!calledslots.length && (retrycount < mps.hb.initialslotretrylimit || document.readyState !== 'complete')) {
    mps._d('[mps:hb:a9] SKIP A9 slot build (waiting for _slotscalled ' + (retrycount + 1) + ')');
    setTimeout(function() {
      mps.hb.a9InitBids(retrycount + 1);
    }, mps.hb.initialslotretryinterval);
    return false;
  }
  mps.hb.a9slots = [];
  mps.hb.a9BuildSlots();
...
/**
 * mps.hb.a9BuildSlots
 * Loop through mps._slotscalled and define a9slots
 * {return} {mps.hb.a9slots|Array}
 */
mps.hb.a9BuildSlots = function () {
  const loadset = mps._loadset || 0;
  const a9slotids = [];
  const calledslots = mps.hb.getCalledSlotNames(loadset);
  const validSlots = mps.hb.getValidSlotObjects(calledslots, loadset);
  mps.hb.a9slots = mps.hb.a9slots || [];
  mps.hb.nohb.a9 = [];
  for (const { name: i, adobj } of validSlots) {
    const slot = {};
    const targ = adobj.getTargetingMap();
    if (targ.nohb?.includes('a9')) {
      mps.hb.nohb.a9.push(i);
      continue;
    }
    slot.slotID = adobj.getSlotElementId();
    slot.slotName = i;
    if (mps.getResponsiveSet && mps.responsiveslots?.[loadset]?.[i]) {
    ...
    const slotsizes = slot.sizes || [];
    let approvedSizesOnly = true;
    let unapprovedSize = "";
    let i_slotsizes = 0;
    while (slotsizes[i_slotsizes]) {
      const sz = slotsizes[i_slotsizes];
      if (typeof(sz) != 'object') {
    ...
    if ( !a9slotids.includes(slot.slotID) ) {
...
mps.hb.a9FetchBids = function() {
  ...
  apstag.fetchBids({
    ...
    try {
      ...
      const adcount = mps.hb.a9slots.length || 1;
      ...
      mps._d('^[mps:hb:a9] A9 fetchBids callback error: ' + err.message);
      mps._errorCallback(err, 'mps.hb.a9FetchBids.callback');
    } finally {
      mps.hb._a9ready = 1;
      mps.hb.startAdExecution();
    }
...
mps.hb.getSlotSizes = function(adobj) {
  const sizeobj = adobj.getSizes();
  const sizes = [];
  for (const sizeitem of sizeobj) {
    const onesize = mps.getValueWithoutKey(sizeitem);
    sizes.push([onesize[0], onesize[1]]);
  }
  mps._d('[mps:hb] getSlotSizes: ' + adobj.getSlotElementId() + ' returned ' + sizes.length + ' size(s)');
  return sizes;
};
/**
 * mps.hb.bidWonHandler
 * Registered once via pbjs.onEvent('bidWon') in executeCall.
 * When useBidCache is enabled, marks the winning bid as used so it is evicted
 * from the cache and the next auction treats that ad unit as fresh.
 * Falls back to the legacy setTargetingForGPTAsync timer if markWinningBidAsUsed
 * is not available in the current Prebid build.
 * Controlled by mps.hb.mgresetonwin (default: true).
 * @param {object} args - Prebid bidWon event args
 */
mps.hb.bidWonHandler = function(args) {
  if (mps.hb.mgresetonwin) {
    if (typeof pbjs.markWinningBidAsUsed === 'function') {
      mps._d('[mps:hb:mg] Bid won, marking as used: ' + args.adUnitCode);
      pbjs.markWinningBidAsUsed({ adUnitCode: args.adUnitCode });
    } else {
      mps._d('[mps:hb:mg] Bid won, resetting targeting (fallback): ' + args.adUnitCode);
      setTimeout(function() {
        pbjs.setTargetingForGPTAsync(args.adUnitCode);
      }, 500);
    }
  } else {
    mps._d('[mps:hb:mg] Bid won, mgresetonwin=false, skipping reset: ' + args.adUnitCode);
  }
};
/**
 * mps.hb.startAdExecution
 * Main execution gate for all HB exchanges.
 *
 * Called by every vendor load callback (loadMG, loadA9 via a9FetchBids, loadDV timeout)
 * and by timeout safety-nets. Each caller triggers a re-check of the readiness gates
 * below. The function only proceeds once ALL gates pass on the same invocation:
 *
 *   Gate 1 – _mgready   : Magnite JS loaded OR mgtimeout fired
 *   Gate 2 – _dvready   : DoubleVerify JS loaded OR dvtimeout fired
 *   Gate 3 – _a9ready   : A9 fetchBids callback returned OR no A9 slots
 *   Gate 4 – adserverCalled : prevents duplicate execution on the same loadset
 *   Gate 5 – _kvinsert.pending : waits for async key-value inserts (e.g. Admantx)
 *   Gate 6 – _slotscalled : retries until at least one GPT slot has been defined
 *
 * Once all gates pass, it queues the Magnite auction (pbjs.rp.requestBids) whose
 * callback triggers initialAdExecution → googletag.pubads().refresh().
 *
 * @param {number} [retrycount] - internal retry counter for slot-wait loop
 * @return {boolean}
 */
mps.hb.startAdExecution = function(retrycount) {
  const loadset = mps._loadset;
  const calledslots = mps.hb.getCalledSlotNames(loadset);
  retrycount = retrycount || 0;
  mps.hb.previousloadset = loadset;
  // Gate 1: Magnite script loaded or timed out
  if (mps.hb.mgid && !mps.hb._mgready) {
    mps._d('[mps:hb:mg] SKIP startAdExecution (waiting for mgready)');
    return false;
  }
  // Gate 2: DoubleVerify script loaded or timed out
  if (mps.hb.dvid && !mps.hb._dvready) {
    mps._d('[mps:hb:dv] SKIP startAdExecution (waiting for dvready)');
    return false;
  }
  // Gate 3: A9 fetchBids callback returned
  if (mps.hb.a9pubid && !mps.hb._a9ready) {
    mps._d('[mps:hb:a9] SKIP startAdExecution (waiting for a9ready)');
    return false;
  }
  // Gate 4: prevent duplicate execution on the same loadset
  ...
  // Gate 5: wait for async key-value inserts (e.g. Admantx)
  ...
  // Gate 6: retry until at least one GPT slot has been defined
  if (!calledslots.length && (retrycount < mps.hb.initialslotretrylimit || document.readyState !== 'complete')) {
    mps._d('[mps:hb] SKIP startAdExecution (waiting for _slotscalled ' + (retrycount + 1) + ')');
    setTimeout(function() {
      mps.hb.startAdExecution(retrycount + 1);
    }, mps.hb.initialslotretryinterval);
    return false;
  }
  mps._queue.gptloaded.push(function() {
    const initialobs = [];
    const bidobs = [];
    const calledslotnames = mps.hb.getCalledSlotNames(loadset);
    ...
    mps.hb.loadedads = 1;
    const validSlots = mps.hb.getValidSlotObjects(calledslotnames, loadset);
    for (const { name: sloti, adobj: slotiv } of validSlots) {
      mps.hb._initialads[loadset].push(sloti);
      const exclude = slotiv.getCategoryExclusions() || [];
      if (exclude?.includes(mps.hb.CAT_EXCLUSION_HB)) {
    ...
    // Include _oop in the initial refresh (skip HB bidding).
    // getValidSlotObjects filters _oop out of bid auctions, but OOP still
    // needs googletag.pubads().refresh() when disableInitialLoad is active.
    if (calledslotnames.indexOf('_oop') > -1 && mps._advars[loadset]?.['_oop']) {
      var oopAdobj = gpt[mps._advars[loadset]['_oop']];
      if (oopAdobj?.getSlotElementId) {
        initialobs.push(oopAdobj);
        mps.hb._initialads[loadset].push('_oop');
        mps.hb._gptrefreshes.initial.push(oopAdobj.getSlotElementId());
        mps._d('[mps:hb] Including _oop in initial refresh (no HB bids)');
      }
    }
    ...
    if (mps.hb.adserverCalled) {
      ...
      /**
         * bidsBackHandler — the Magnite auction callback (initial page load).
         *
         * pbjs.rp.requestBids sends `bidobs` (eligible GPT slot objects) to
         * Magnite's Prebid auction. When the auction completes (or its internal
         * timeout fires), this callback runs. It calls initialAdExecution which
         * triggers googletag.pubads().refresh(initialobs) to render ads.
         *
         * `bidobs`     = slots WITH HB (no "headerbid" category exclusion)
         * `initialobs` = ALL called slots (bidobs + excluded)
         *
         * Both sets are refreshed together so excluded slots still render; they
         * just skip the Prebid auction.
         *
         * If Magnite is not configured (no mgid) or pbjs failed to load, we
         * fall through to initialAdExecution directly — ads render without bids.
         */
	      if (mps.hb.mgid && pbjs && bidobs.length) {
        mps._d('[mps:hb:mg] Requesting Bids for ' + bidobs.length + ' slot(s)');
        mps.hb.markTiming('auction_start_delay_ms', Date.now() - mps.hb._executionStartTs);
        pbjs.que.push(function() {
	          pbjs.rp.requestBids({
	              gptSlotObjects: bidobs,
	            callback: function() {
							  try {
							    mps.hb.initialAdExecution(initialobs);
							  } catch (err) {
							    mps._d('^[mps:hb:mg] bidsBackHandler error: ' + err.message);
							    mps._errorCallback(err, 'mps.hb.bidsBackHandler');
							  }
	            }
	          });
			  });
...
mps.hb.initialAdExecution = function(initialobs) {
  if (!initialobs?.length) {
    return false;
  }
  mps._d('[mps:hb] Initial Ad Execution: ' + initialobs.map(function(s) { return s.getSlotElementId(); }).join(', '));
  // Clear stale hb_* targeting and apply fresh bids before GPT consumes them
  pbjs.setTargetingForGPTAsync();
  // Snapshot each slot's Prebid targeting before GPT consumes it
  for (const slot of initialobs) {
    mps.hb._snapshotSlotBid(slot, 'initial');
  }
...
/**
 * mps.hb.refreshOverrideCallback
 * Refresh-time bid path #2 of 2: late-rendered ad callback.
 *
 * Called by header-bidding-ad-execute-callback.js (_adexecuteCallback) for
 * slots that render AFTER the initial page-load auction has already fired.
 * When a slot's adexecuteCallback finds that loadedads=1 (auction already
 * ran) and the slot is HB-eligible, it fires a per-slot pbjs.rp.requestBids
 * with this function as the callback. This ensures late slots still get
 * Prebid bids without triggering a full-page re-auction.
 *
 * Contrast with mps.refreshAdsPreCallback (path #1) which handles
 * explicit mps.refreshAds() calls from the page.
 *
 * @param {(object|array)} adSlots - GPT slot object(s) to refresh
 * @return {boolean}
 */
mps.hb.refreshOverrideCallback = function(adSlots) {
  try {
    let refreshSlots = adSlots;
    if (!refreshSlots) {
      return false;
    }
    if ('[object Array]' !== Object.prototype.toString.call(refreshSlots)) {
      refreshSlots = [refreshSlots];
    }
    mps._d('^[mps:hb:mg] refreshOverrideCallback called: ' + refreshSlots);
    // Clear stale hb_* targeting and apply fresh bids
    pbjs.setTargetingForGPTAsync();
    // Snapshot each slot's Prebid targeting before GPT consumes it
    for (const slot of refreshSlots) {
      mps.hb._snapshotSlotBid(slot, 'refreshOverride');
    }
    googletag.pubads().refresh(refreshSlots, {
      changeCorrelator: false
    });
  } catch (err) {
    mps._d('^[mps:hb:mg] refreshOverrideCallback error: ' + err.message);
    mps._errorCallback(err, 'mps.hb.refreshOverrideCallback');
  }
};
...
//--> TIMED AD REFRESH CONFIG [nbcnews-bento]
/*=======TIMED REFRESH CONFIG=======*/
/* Define refresh intervals (in seconds) per slot. */
/* Optionally override intervals per responsive breakpoint. */

mpsopts.timedrefresh = {
  // Base intervals (required) - applies to all breakpoints unless overridden
  slots: {
    topbanner: 30,
    boxflex: 30,
    boxrail: 30,
...
mps.execute_refresh_js = function() {
   ...
   * Viewability-gated refresh: only refresh slots that have been viewed.
   * @member {object} smartrefresh
   * @memberOf mpsopts
   * @property {boolean} enabled - Enable/disable smart refresh (default: false)
   * @property {number}  viewtime - Minimum viewtime in ms before slot qualifies for refresh (default: 0, disabled)
   * @property {number}  maxrefresh - Maximum refreshes per slot per session (default: 0, unlimited)
   */
  mpsopts.smartrefresh = mpsopts.smartrefresh || {};
  mpsopts.smartrefresh.enabled = (typeof mpsopts.smartrefresh.enabled !== 'undefined') ? mpsopts.smartrefresh.enabled : false;
  mpsopts.smartrefresh.viewtime = (typeof mpsopts.smartrefresh.viewtime !== 'undefined') ? mpsopts.smartrefresh.viewtime : 0;
  mpsopts.smartrefresh.maxrefresh = (typeof mpsopts.smartrefresh.maxrefresh !== 'undefined') ? mpsopts.smartrefresh.maxrefresh : 0;
  
  /**
   * Per-slot refresh counter for smart refresh cap.
   * Tracks how many times each slot has been refreshed this session.
   * @member {object} __refreshcount
   * @memberOf mps
   */
  mps.__refreshcount = mps.__refreshcount || {};
  
  /**
   * Check if a slot qualifies for smart refresh.
   * Returns true if the slot has been viewed for the required duration
   * and hasn't exceeded the max refresh cap.
   * @param {string} slotname - The slot name (e.g. 'topbanner')
   * @param {number} loadset - The loadset index
   * @return {boolean} Whether the slot should be refreshed
   */
  mps.smartRefreshCheck = function(slotname, loadset) {
    if (!mpsopts.smartrefresh.enabled) {
      return true; // Smart refresh disabled, allow all refreshes
    }
    var ls = (typeof loadset !== 'undefined') ? loadset : mps._loadset;
    // Check refresh cap
    mps.__refreshcount[ls] = mps.__refreshcount[ls] || {};
    var count = mps.__refreshcount[ls][slotname] || 0;
    if (mpsopts.smartrefresh.maxrefresh > 0 && count >= mpsopts.smartrefresh.maxrefresh) {
      mps._debug('[mps:SmartRefresh] SKIP ' + slotname + ' - max refresh cap reached (' + count + '/' + mpsopts.smartrefresh.maxrefresh + ')');
      return false;
    }
    // Check viewability - two data structures depending on whether the slot
    // was previously refreshed (see gpt-shared.js _adViewCallback):
    //   Initial load:  mps._adloads[ls][slot]._mps._viewed / ._viewtime
    //   After refresh:  mps._adloads['R' + ls][slot][index]._viewed / ._viewtime
    var adData = null;
    var _rls = 'R' + ls;
    // Check refreshed-ad structure first (most recent data)
    if (mps._adloads && mps._adloads[_rls] && mps._adloads[_rls][slotname]) {
      var _rdata = mps._adloads[_rls][slotname];
      if (typeof _rdata === 'object') {
        // Find the latest entry (highest index)
        var _rkeys = mps._keys(_rdata);
        if (_rkeys.length) {
          var _lastKey = _rkeys[_rkeys.length - 1];
          if (_rdata[_lastKey] && typeof _rdata[_lastKey]._viewed !== 'undefined') {
            adData = _rdata[_lastKey];
          }
        }
      }
    }
    // Fall back to initial load structure
    if (!adData && mps._adloads && mps._adloads[ls] && mps._adloads[ls][slotname] && mps._adloads[ls][slotname]._mps) {
      adData = mps._adloads[ls][slotname]._mps;
    }
  
    if (adData) {
      if (adData._viewed !== 1) {
        mps._debug('[mps:SmartRefresh] SKIP ' + slotname + ' - not viewed');
        return false;
      }
      if (mpsopts.smartrefresh.viewtime > 0 && adData._viewtime < mpsopts.smartrefresh.viewtime) {
        mps._debug('[mps:SmartRefresh] SKIP ' + slotname + ' - viewtime ' + adData._viewtime + 'ms < ' + mpsopts.smartrefresh.viewtime + 'ms threshold');
        return false;
      }
      mps._debug('[mps:SmartRefresh] PASS ' + slotname + ' - viewed=' + adData._viewed + ', viewtime=' + adData._viewtime + 'ms, count=' + count);
      return true;
    }
    // No viewability data available - allow refresh (graceful fallback)
    mps._debug('[mps:SmartRefresh] PASS ' + slotname + ' - no viewability data, allowing refresh');
    return true;
  };
  ...
  // Ensure _detect namespace exists even if lazyload.js hasn't executed yet.
  // lazyload.js is conditionally called (requires dart_lazy_load), but refresh.js
  // always loads and references mps._detect when mpsopts.activerefresh > 0.
  mps._detect = mps._detect || {};
  mps._detect._refresh = mps._detect._refresh || {};
  // Fallback viewport check: when lazyload.js hasn't set up detection,
  // treat all slots as in-viewport so they refresh normally.
  if (typeof mps._detect.viewport !== 'function') {
    mps._detect.viewport = function() { return true; };
  }
  
  /**
   * mps.refreshDetect refreshes detected display ads
  ...
  mps.refreshDetect = function(slotname, loadset, _skipresponsive) {
    ...
    if((typeof loadset !== 'undefined' || typeof loadset === 'number') && typeof slotname !== 'object') {
      ...
      if(slotElem && slotElem.dataset && slotElem.dataset.mpsSlot){
        slotElemName =  slotElem.dataset.mpsSlot;
      } else if(slotElem && slotElem.getAttribute && slotElem.getAttribute('data-mps-slot')){
   ...
   * @param {boolean} _skiphb - if true, skip header bidding pre-hook for this refresh
   * @return {boolean}
   */
  mps.refreshAds = function (slotname,loadset,changecorrelator,_skipresponsive,_interval,_skiphb) {
    var gptopts = {};
    ...
    if(mps.__refreshing) {
      _interval = setInterval(function(){
        mps.refreshAds(slotname,loadset,changecorrelator,_skipresponsive,_interval,_skiphb);
        return false;
    ...
    if((!window.googletag || !googletag.apiReady) || typeof(mps.pagevars.dart_mode) == "undefined" || mps.pagevars.dart_mode != "gpt-asynchronous") {
      ...
      if(typeof(slotname) != 'object') {
        ...
        for (var i in slotname) {
          ...
          for (var ls in mps._advars) {
            ...
            var _advarKeys = mps._keys(mps._advars[_ls]); // Cache keys once per loadset
            for (var cs=0; cs<_advarKeys.length; cs++) {
              if (typeof(slotname[i])== 'string'  && _advarKeys[cs].indexOf(slotname[i] + '-C') > -1) {
                ...
                clonevar = (typeof(slotname[i]) == 'object') ? slotname[i] : mps.getSlot(_advarKeys[cs], _ls);
                if (typeof(clonevar) == 'object' && !disabledslot) {
                  if (!mps.lazyload) {
                    slotvars.push(clonevar);
                    _slotvars[_loadset].push(clonevar);
                  } else {
                    if (mps.refreshDetect(_advarKeys[cs], _ls, _skipresponsive)) {
                      slotvars.push(clonevar);
                      _slotvars[_loadset].push(clonevar);
                    }
                  }
      ...
      var _execute = function(svk,_slotvars,_set, clone, _skiphb) {
        ...
        setTimeout(function() {
          ...
          // Smart Refresh: filter out slots that haven't been viewed or exceeded cap
          if (mpsopts.smartrefresh.enabled && _updatedslotvars.length) {
            var _filteredvars = [];
            for (var sr = 0; sr < _updatedslotvars.length; sr++) {
              if (typeof _updatedslotvars[sr] === 'object' && typeof _updatedslotvars[sr].getSlotElementId === 'function') {
                var _srId = _updatedslotvars[sr].getSlotElementId();
                // Extract slot name from GPT div id (e.g. 'div-gpt-topbanner-123' → 'topbanner')
                var _srName = _srId.replace(/^div-gpt-/, '').replace(/-\d+$/, '');
                if (mps.smartRefreshCheck(_srName, _set)) {
                  _filteredvars.push(_updatedslotvars[sr]);
                  // Increment refresh counter
                  mps.__refreshcount[_set] = mps.__refreshcount[_set] || {};
                  mps.__refreshcount[_set][_srName] = (mps.__refreshcount[_set][_srName] || 0) + 1;
                }
              } else {
                _filteredvars.push(_updatedslotvars[sr]); // Non-standard slot, allow through
              }
            }
            mps._debug('[mps:SmartRefresh] Filtered: ' + _updatedslotvars.length + ' → ' + _filteredvars.length + ' slots');
            _updatedslotvars = _filteredvars;
            if (!_updatedslotvars.length) {
              mps._debug('[mps:SmartRefresh] No slots qualified for refresh, skipping GPT call');
              return;
            }
          }
          // Refresh ads in loadset.
          mps._gptcmd.push(function() {
            // Pre-hook: allow header bidding to fetch new bids before GPT refresh
            if (!_skiphb && typeof mps.refreshAdsPreCallback === 'function') {
              var handled = mps.refreshAdsPreCallback(_updatedslotvars, gptopts);
              if (handled) return;
            }
      ...
      for(var svk in _slotvars) {
        ...
        _execute(svk,_slotvars,_set, undefined, _skiphb);
      }
      setTimeout(function(){
...
/* js/advertising/timedrefresh.js */

mps.execute_timedrefresh_js = function() {
  /**
   * Timed Ad Refresh with Responsive Breakpoint Support
   * Periodically refreshes ad slots based on mpsopts.timedrefresh config.
   * Supports breakpoint-specific intervals via mpsopts.timedrefresh.breakpoints.
   * Refresh is gated by window focus and viewport visibility (clone-aware).
   * Deferred refreshes catch up on focus regain or scroll back into viewport.
   *
   * Config format (set via Global Defaults > Timed Ad Refresh Config):
   *   mpsopts.timedrefresh = {
   *     slots: { topbanner: 30, boxflex: 30 },
   *     breakpoints: { 'large': { topbanner: 15 }, '0': { topbanner: 0 } }
   *   };
   *
   * @requires mps.refreshAds (refresh.js)
   * @requires mps.inViewport (mpstools.js)
   * @requires mps.getResponsiveSet (mpstools.js)
   * @requires mps._queue.gptloaded, mps._queue.adload
   */
  (function() {
    var tr = mps._timedRefresh = mps._timedRefresh || {};
    var config = (typeof mpsopts !== 'undefined' && mpsopts.timedrefresh && mpsopts.timedrefresh.slots)
      ? mpsopts.timedrefresh : null;
  
    if (!config || typeof config !== 'object' || !config.slots || typeof config.slots !== 'object') {
      return;
    }
  
    tr.states = tr.states || {};
    tr.currentBreakpoint = null;
    tr.viewportCatchupDebounceMs = tr.viewportCatchupDebounceMs || 100;
    tr.breakpointDebounceMs = tr.breakpointDebounceMs || 300;
  
    // Get current responsive breakpoint
    var getCurrentBreakpoint = function() {
      if (typeof mps.getResponsiveSet === 'function') {
        var bp = mps.getResponsiveSet();
        return (bp === false) ? null : String(bp);
      }
      return null;
    };
  
    // Resolve interval for a slot at the current breakpoint
    var getIntervalForSlot = function(slot) {
      var breakpoint = getCurrentBreakpoint();
  
      // Check breakpoint-specific override first
      if (breakpoint && config.breakpoints && typeof config.breakpoints === 'object' &&
          config.breakpoints[breakpoint] && typeof config.breakpoints[breakpoint][slot] !== 'undefined') {
        return parseInt(config.breakpoints[breakpoint][slot], 10);
      }
  
      // Fall back to default slot interval
      if (config.slots && typeof config.slots[slot] !== 'undefined') {
        return parseInt(config.slots[slot], 10);
      }
  
      return null;
    };
  
    var slotLabel = function(state) {
      var secs = state.intervalMs ? Math.round(state.intervalMs / 1000) : null;
      var bp = tr.currentBreakpoint || '';
      var label = state.slot;
      if (secs) {
        label += ':' + secs;
      }
      if (bp) {
    ...
    var isLazySlot = function(slot, loadset) {
      return !!(mps.lazyload && mps.lazyload[loadset] && mps.lazyload[loadset].indexOf(slot) > -1);
    };
  
    var hasWindowFocus = function() {
      if (typeof document.hasFocus === 'function') {
        return document.hasFocus();
      }
      return !!(mps._activerefresh && mps._activerefresh._focus);
    };
  
    var clearSlotTimer = function(state) {
    ...
    var debounce = function(func, waitMs) {
      var timeoutId = null;
      return function() {
        if (timeoutId) {
    ...
    var getSlotElement = function(state) {
      var slotId = mps._adslots && mps._adslots[state.loadset] && mps._adslots[state.loadset][state.slot];
      return slotId ? document.getElementById(slotId) : false;
    };
  
    // Clone-aware viewport check: returns true if base slot OR any clone is in viewport.
    var isSlotInViewport = function(state) {
      if (typeof mps.inViewport !== 'function') {
        return false;
      }
      var slotElem = getSlotElement(state);
      if (slotElem && mps.inViewport(slotElem, false)) {
        return true;
      }
      if (mps._adslots && mps._adslots[state.loadset]) {
        var prefix = state.slot + '-C';
        for (var key in mps._adslots[state.loadset]) {
          if (mps._adslots[state.loadset].hasOwnProperty(key) && key.indexOf(prefix) === 0) {
            var cloneElem = document.getElementById(mps._adslots[state.loadset][key]);
            if (cloneElem && mps.inViewport(cloneElem, false)) {
              return true;
            }
          }
        }
    ...
    var markOverdue = function(state, reason) {
      state.overdue = true;
      state.nextDueAt = null;
      state.lastMissedReason = reason || 'unknown';
      mps._debug('[mps:timedRefresh] Deferred refresh while ' + state.lastMissedReason + ': ' + slotLabel(state) + '.');
    };
  
    var scheduleNextRefresh = function(state, delayMs) {
      clearSlotTimer(state);
      state.nextDueAt = Date.now() + delayMs;
      state.timerId = setTimeout(function() {
        state.timerId = null;
  
        if (!hasWindowFocus()) {
          markOverdue(state, 'unfocused');
          return false;
        }
  
        if (!isSlotInViewport(state)) {
          markOverdue(state, 'out of viewport');
          return false;
        }
  
        state.lastRefreshAt = Date.now();
        state.lastMissedReason = null;
        mps.refreshAds(state.slot, state.loadset);
        scheduleNextRefresh(state, state.intervalMs);
        return true;
      }, delayMs);
    };
  
    var refreshEligibleOverdueStates = function(source) {
      if (!hasWindowFocus()) {
        return false;
      }
  
      var overdueByLoadset = {};
      var refreshedStates = [];
  
      for (var loadset in tr.states) {
        if (!tr.states.hasOwnProperty(loadset)) {
          continue;
        }
        for (var slot in tr.states[loadset]) {
          if (!tr.states[loadset].hasOwnProperty(slot)) {
            continue;
          }
          var state = tr.states[loadset][slot];
          if (!state || !state.hasLoaded || !state.overdue || state.disabled) {
            continue;
          }
          if (!isSlotInViewport(state)) {
            continue;
          }
          overdueByLoadset[loadset] = overdueByLoadset[loadset] || [];
          overdueByLoadset[loadset].push(slot);
          refreshedStates.push(state);
        }
      }
  
      if (!refreshedStates.length) {
        return false;
      }
  
      for (var i = 0; i < refreshedStates.length; i++) {
        refreshedStates[i].overdue = false;
        refreshedStates[i].lastMissedReason = null;
        refreshedStates[i].lastRefreshAt = Date.now();
      }
  
      for (var dueLoadset in overdueByLoadset) {
        if (overdueByLoadset.hasOwnProperty(dueLoadset) && overdueByLoadset[dueLoadset].length) {
          var labels = [];
          for (var s = 0; s < overdueByLoadset[dueLoadset].length; s++) {
            var st = tr.states[dueLoadset][overdueByLoadset[dueLoadset][s]];
            labels.push(st ? slotLabel(st) : overdueByLoadset[dueLoadset][s]);
          }
          mps._debug('[mps:timedRefresh] Catch-up refresh on ' + source + ' for loadset ' + dueLoadset + ': ' + labels.join(', '));
          mps.refreshAds(overdueByLoadset[dueLoadset], parseInt(dueLoadset, 10));
        }
      }
  
      for (var j = 0; j < refreshedStates.length; j++) {
        scheduleNextRefresh(refreshedStates[j], refreshedStates[j].intervalMs);
      }
  
      return true;
    };
  
    var handleFocusCatchup = function() {
      refreshEligibleOverdueStates('focus');
    };
  
    var handleViewportCatchup = debounce(function() {
      refreshEligibleOverdueStates('scroll/resize');
    }, tr.viewportCatchupDebounceMs);
  
    var handleAdload = function(eo) {
      var info = eo && eo._mps;
      if (!info || info._slot === '_oop') {
        return false;
      }
  
      var loadset = (typeof info.loadset === 'number' && !isNaN(info.loadset)) ? info.loadset : 0;
      var slot = info._slot;
      var state = tr.states[loadset] && tr.states[loadset][slot];
  
      if (!state) {
        return false;
      }
  
      if (info._refreshed === 0) {
        ...
        // Don't schedule if disabled at current breakpoint
        if (!state.disabled) {
          scheduleNextRefresh(state, state.intervalMs);
          mps._debug('[mps:timedRefresh] Started refresh timer from first adload for ' + slotLabel(state) + ' on loadset ' + loadset + '.');
        }
      }
    ...
    var handleBreakpointChange = function() {
      ...
      if (String(newBreakpoint) === String(tr.currentBreakpoint)) {
        return;
      }
  
      mps._debug('[mps:timedRefresh] Breakpoint changed: ' + (tr.currentBreakpoint || '[none]') + ' → ' + (newBreakpoint || '[none]'));
      tr.currentBreakpoint = newBreakpoint;
  
      // Re-evaluate all slots with new intervals
      for (var loadset in tr.states) {
        if (!tr.states.hasOwnProperty(loadset)) {
          continue;
        }
        for (var slot in tr.states[loadset]) {
          ...
          var state = tr.states[loadset][slot];
          var newIntervalSecs = getIntervalForSlot(slot);
          var newIntervalMs = newIntervalSecs * 1000;
  
          // Slot disabled at this breakpoint (interval = 0 or invalid)
          if (!(newIntervalMs > 0)) {
            if (!state.disabled) {
              clearSlotTimer(state);
              state.disabled = true;
              state.overdue = false;
              mps._debug('[mps:timedRefresh] Disabled ' + slot + ' at breakpoint ' + (newBreakpoint || '[none]'));
            }
          }
          // Slot enabled
          else {
            var wasDisabled = state.disabled;
            var intervalChanged = state.intervalMs !== newIntervalMs;
  
            if (wasDisabled || intervalChanged) {
              state.intervalMs = newIntervalMs;
              state.disabled = false;
  
              // Only reschedule if the slot has already loaded
              if (state.hasLoaded) {
                clearSlotTimer(state);
  
                // If overdue, let viewport/focus catchup handle it
                if (!state.overdue) {
                  scheduleNextRefresh(state, newIntervalMs);
                }
  
                var secs = Math.round(newIntervalMs / 1000);
                if (wasDisabled) {
                  mps._debug('[mps:timedRefresh] Enabled ' + slot + ':' + secs + ':' + (newBreakpoint || '[none]'));
                } else {
                  mps._debug('[mps:timedRefresh] Updated ' + slot + ' to ' + secs + 's at breakpoint ' + (newBreakpoint || '[none]'));
                }
              }
            }
          }
    ...
    tr.bindFocusListener = tr.bindFocusListener || function() {
      if (tr.focusListenerBound) {
        return;
      }
      tr.focusListenerBound = true;
      if (window.addEventListener) {
        window.addEventListener('focus', handleFocusCatchup);
      } else if (window.attachEvent) {
        window.attachEvent('onfocus', handleFocusCatchup);
      }
    };
  
    tr.bindViewportListener = tr.bindViewportListener || function() {
      if (tr.viewportListenerBound) {
        return;
      }
      tr.viewportListenerBound = true;
      if (window.addEventListener) {
        window.addEventListener('scroll', handleViewportCatchup);
        window.addEventListener('resize', handleViewportCatchup);
      } else if (window.attachEvent) {
        window.attachEvent('onscroll', handleViewportCatchup);
        window.attachEvent('onresize', handleViewportCatchup);
      }
    };
  
    tr.bindBreakpointListener = tr.bindBreakpointListener || function() {
      if (tr.breakpointListenerBound) {
        return;
      }
      tr.breakpointListenerBound = true;
  
      var handleResize = debounce(handleBreakpointChange, tr.breakpointDebounceMs);
    ...
    tr.bindAdloadListener = tr.bindAdloadListener || function() {
      if (tr.adloadListenerBound || !mps._queue || !mps._queue.adload || typeof mps._queue.adload.push !== 'function') {
        return;
      }
      tr.adloadListenerBound = true;
      mps._queue.adload.push(handleAdload);
    };
  
    var initTimedRefresh = function() {
      tr.bindFocusListener();
      tr.bindViewportListener();
      tr.bindBreakpointListener();
      tr.bindAdloadListener();
  
      // Initialize current breakpoint
      tr.currentBreakpoint = getCurrentBreakpoint();
      if (tr.currentBreakpoint) {
        mps._debug('[mps:timedRefresh] Initial breakpoint: ' + tr.currentBreakpoint);
      }
  
      var loadset = (typeof mps._loadset === 'number' && !isNaN(mps._loadset)) ? mps._loadset : 0;
      tr.states[loadset] = tr.states[loadset] || {};
  
      for (var slot in config.slots) {
        if (!config.slots.hasOwnProperty(slot)) {
          continue;
        }
  
        var intervalSecs = getIntervalForSlot(slot);
  
        if (intervalSecs === null) {
          mps._debug('[mps:timedRefresh] Skip ' + slot + ': no config.');
          continue;
        }
  
        var intervalMs = intervalSecs * 1000;
        var disabled = !(intervalMs > 0);
  
        if (!mps._adslots || !mps._adslots[loadset] || !mps._adslots[loadset][slot]) {
          if (!disabled) {
            mps._debug('[mps:timedRefresh] Skip ' + slot + ':' + intervalSecs + ': slot not found on loadset ' + loadset + '.');
          }
          continue;
        }
  
        if (tr.states[loadset][slot]) {
          clearSlotTimer(tr.states[loadset][slot]);
        }
  
        tr.states[loadset][slot] = {
          slot: slot,
          loadset: loadset,
        ...
        if (disabled) {
          mps._debug('[mps:timedRefresh] Slot ' + slot + ' is disabled at breakpoint ' + (tr.currentBreakpoint || '[none]'));
        } else if (isLazySlot(slot, loadset)) {
          mps._debug('[mps:timedRefresh] Registered lazy slot ' + slot + ':' + intervalSecs + ':' + (tr.currentBreakpoint || '[none]') + ' on loadset ' + loadset + '; refresh timer will start on first adload.');
        } else {
          tr.states[loadset][slot].hasLoaded = true;
          scheduleNextRefresh(tr.states[loadset][slot], intervalMs);
          mps._debug('[mps:timedRefresh] Registered ' + slot + ':' + intervalSecs + ':' + (tr.currentBreakpoint || '[none]') + ' on loadset ' + loadset + '.');
        }
      }
    ...
    if (mps._queue && mps._queue.gptloaded && typeof mps._queue.gptloaded.push === 'function') {
      mps._queue.gptloaded.push(initTimedRefresh);
    } else {
      initTimedRefresh();
    }
  })();
